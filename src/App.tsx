...
export default function App() {
  ...
  const getGlobalSearchItems = () => {
    ...
    // 5. Contracts
    try {
      const savedContracts = localStorage.getItem('sio_contracts_v2');
      let contractList = [];
  ...
  const getFilteredSearchResults = () => {
    ...
    const matched = allItems.filter(item => 
      ...
      (item.badge && (item.badge && item.badge.toLowerCase().includes(query)))
    );
  ...
  const syncData = async () => {
    try {
      const [
        ...
        fetch('/api/users').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }),
        fetch('/api/forms').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }),
        fetch('/api/requests').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }),
        
        fetch(currentUser ? `/api/chats?userId=${currentUser.id}` : '/api/chats').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }),
        fetch('/api/chats/groups').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }).catch(() => []),
        fetch(currentUser ? `/api/notifications?userId=${currentUser.id}` : '/api/notifications').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } }),
        fetch('/api/payments').then(async res => { if (!res.ok) throw new Error('API Error'); const text = await res.text(); try { return JSON.parse(text); } catch(e) { console.error('Failed to parse JSON for ' + res.url, text.substring(0, 50)); return []; } })
      ]);
  ...
  const handleAnalyzeAI = async (formTitle: string, values: Record<string, any>) => {
    try {
      ...
      const text_ret = await res.text(); if (!text_ret) return {}; try { return JSON.parse(text_ret); } catch(e) { return {}; }
    } catch (err) {
              ...
              {globalSearchQuery.trim() !== '' && (
                <div className="absolute top-full mt-2 right-0 w-80 sm:w-96 bg-white border border-[#e2eae8] rounded-2xl shadow-xl overflow-hidden z-50 font-sans animate-in fade-in slide-in-from-top-1 duration-150">
                  ...
                  <div className="max-h-[360px] overflow-y-auto divide-y divide-slate-100 custom-scrollbar">
                    ...
                    <div className="p-2">
                      <div className="px-2 py-1 text-[9px] font-extrabold text-slate-500 bg-[#f4f7f6] rounded-md uppercase tracking-wider mb-1">
