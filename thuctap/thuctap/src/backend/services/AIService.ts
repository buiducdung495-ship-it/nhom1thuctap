...
- Trả về đúng định dạng JSON, không kèm giải thích.`;
  ...
  static async chatWithAI(userMessage: string, chatHistory: { role: 'user' | 'model'; text: string }[]): Promise<string> {
    const systemInstruction = `Bạn là Trợ lý AI Thông minh (AI HR Bot) tích hợp trong hệ thống quản lý công việc và hành chính nội bộ của công ty.
Nhiệm vụ của bạn là hỗ trợ trực tuyến, giải đáp và tư vấn trực tiếp cho nhân viên.

YÊU CẦU & CHỨC NĂNG:
- Luôn ưu tiên trả lời đúng trọng tâm dựa vào nghiệp vụ, quy định của hệ thống.
- Cung cấp thông tin thực tế, chính xác và chuyên nghiệp.
- Khi người dùng gặp sự cố hệ thống: Đưa ra các hướng giải quyết tạm thời (ví dụ tải lại trang, xóa cache) và hướng dẫn họ báo cáo vấn đề gây lỗi hệ thống qua bộ phận Dev.
...
- Nếu câu hỏi nằm ngoài phạm vi công việc công ty hoặc hệ thống, hãy từ chối khéo léo.
Luôn trả lời thân thiện, súc tích và bằng tiếng Việt.`;
    ...
    return 'Cảm ơn câu hỏi của bạn. Quy trình công ty quy định rõ ràng: các đơn từ đều được tự động hóa phân cấp duyệt theo mức độ nghiêm trọng. Bạn có cần hướng dẫn tạo biểu mẫu hay duyệt đơn không?';
  }
}
