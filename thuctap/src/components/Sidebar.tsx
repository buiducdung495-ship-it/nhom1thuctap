...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const ALL_MENU_ITEMS = [
    { id: 'dashboard', label: 'Phân tích quy trình', icon: LayoutDashboard, section: 'main' },
    { id: 'tasks', label: 'Dự án & Nhiệm vụ', icon: FolderKanban, section: 'main' },
    { id: 'events', label: 'Lịch & Sự kiện', icon: Calendar, section: 'main' },
    { id: 'user-management', label: 'Nhân sự & Tài khoản', icon: Users, section: 'main' },
    ...
    { id: 'form-builder', label: 'Thiết kế biểu mẫu', icon: PenTool, section: 'extended' },
    { id: 'assets', label: 'Quản lý tài sản', icon: Laptop, section: 'extended' },
    ...
    <aside className={`${isCollapsed ? 'w-16 md:w-16' : 'w-full md:w-68'} bg-white text-slate-700 flex flex-col h-auto md:h-screen sticky top-0 border-r border-[#e2eae8] z-30 font-sans shadow-sm shrink-0 transition-all duration-300`}>
      ...
      <div className={`p-4 ${isCollapsed ? 'px-2 justify-center' : 'justify-between'} border-b border-[#e2eae8] flex items-center h-16 shrink-0 overflow-hidden`}>
        {!isCollapsed ? (
          <div className="flex items-center">
            <div className="bg-[#2f80ed] w-10 h-10 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-md shadow-[#2f80ed]/25 tracking-wider shrink-0">
              JIN
            </div>
            <div className="overflow-hidden transition-all duration-300 flex flex-col justify-center w-36 ml-3 opacity-100">
              <h1 className="text-xs font-extrabold tracking-tight text-[#0a2e24] whitespace-nowrap">HỆ THỐNG JIN</h1>
              <p className="text-[9px] text-[#2f80ed] font-bold uppercase tracking-wider whitespace-nowrap">Quản lý Dự án</p>
            </div>
          </div>
        ) : null}
        <button 
      ...
      <div className="flex-1 p-3 flex flex-col justify-between overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-3 mt-4 pt-4 border-t border-[#e2eae8]">
          ...
          {!isCollapsed ? (
            <div className="bg-[#f3f7ff] border border-blue-100 rounded-2xl p-4 text-center relative overflow-hidden transition-all duration-300">
              ...
              <p className="text-[9px] text-slate-500 mt-1 leading-relaxed">Gặp khó khăn? Chuyên gia JIN sẵn sàng hỗ trợ bạn 24/7.</p>
      ...
      <div className={`p-4 ${isCollapsed ? 'px-2 flex-col space-y-3' : 'flex-row'} border-t border-[#e2eae8] bg-[#f8fafc] flex items-center justify-between shrink-0 transition-all duration-300 relative`}>
        <div 
          ...
          className="flex items-center cursor-pointer hover:bg-slate-100/80 p-1.5 rounded-xl min-w-0 group"
